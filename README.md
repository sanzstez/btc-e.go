...
update_period = 5
```
4. Таймаут запроса от API сервера биржи задается параметром **request_timeout** (указывается в секундах):
```
request_timeout = 5
```

5. Возможность указать количество знаков после запятой для разницы курсов - параметр **exp_signs**:
```
exp_signs = 6
```
6. Служебные параметры **show_logger**  и **fix_window** можно изменить в самой программе.
